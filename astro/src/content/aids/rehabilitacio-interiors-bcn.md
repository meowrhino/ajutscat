---
id: rehabilitacio-interiors-bcn
nombre_ca: Rehabilitar interior · Borsa BCN
nombre_es: Rehabilitar interior · Bolsa BCN
subtitle_ca: "Ajut a la rehabilitació d'interiors per incorporar habitatges a la Borsa de Lloguer de Barcelona"
subtitle_es: "Ayuda a la rehabilitación de interiores para incorporar viviendas a la Bolsa de Alquiler de Barcelona"
categoria: vivienda
subcategoria: rehabilitacio
bdns_id: "880145"
cuantia_ca: "100% del cost · fins a 30.000 €/habitatge (mínim 500 €)"
cuantia_es: "100% del coste · hasta 30.000 €/vivienda (mínimo 500 €)"
cuantia_total_max: 30000
edad_min: 18
edad_max: 120
ingresos_max: null
ambito: barcelona
zonas_especificas:
  - Barcelona ciutat
estado: abierta
fecha_apertura: 2026-01-07
fecha_cierre: 2026-11-30
duracion_ventana_dias: 328
fuente_oficial: https://www.habitatge.barcelona/ca/ajuts-rehabilitacio-interior-habitatges-borsa-lloguer
organismo_ca: "Consorci de l'Habitatge de Barcelona"
organismo_es: Consorcio de la Vivienda de Barcelona
ultima_actualizacion: 2026-08-30
stale: false
requisitos_ca:
  - Habitatge situat a Barcelona ciutat.
  - "Compromís d'incorporar l'habitatge a la Borsa de Lloguer o a programes de cessió."
  - Titularitat de l'habitatge o autorització del propietari.
  - Obres d'interior (no estructurals).
requisitos_es:
  - Vivienda situada en Barcelona ciudad.
  - Compromiso de incorporar la vivienda a la Bolsa de Alquiler o a programas de cesión.
  - Titularidad de la vivienda o autorización del propietario.
  - Obras de interior (no estructurales).
documentacion_ca:
  - Escriptura o títol de propietat
  - "Projecte o memòria d'obra"
  - Pressupost detallat
  - Fotografies de l'estat previ
  - "Compromís d'incorporació a la Borsa"
documentacion_es:
  - Escritura o título de propiedad
  - Proyecto o memoria de obra
  - Presupuesto detallado
  - Fotografías del estado previo
  - Compromiso de incorporación a la Bolsa
historico: []
fetcher_last_ok: 2026-08-30T11:01:54.303Z
---

# Rehabilitar interior · Borsa BCN

Ajut plurianual a propietaris que rehabilitin l'interior d'un habitatge i el cedeixin durant 6 anys a la **Borsa de Lloguer de Barcelona**, a la **Mesa d'emergències** o al programa **Primer la Llar**. Cobreix el 100% del cost, fins a 30.000 €/habitatge (mínim 500 €). Termini: fins al 30 de novembre de 2026; les obres han de finalitzar abans del 30 d'octubre de 2027.

Font oficial: https://www.habitatge.barcelona/ca/ajuts-rehabilitacio-interior-habitatges-borsa-lloguer
