---
id: beca-equitat-universitats
nombre_ca: Beca Equitat
nombre_es: Beca Equitat
subtitle_ca: Reducció del preu de matrícula universitària segons renda familiar
subtitle_es: Reducción del precio de matrícula universitaria según renta familiar
categoria: estudios
subcategoria: beca
bdns_id: ""
cuantia_ca: 20 %–80 % de la matrícula
cuantia_es: 20 %–80 % de la matrícula
cuantia_total_max: null
edad_min: 18
edad_max: 120
ingresos_max: 60000
ambito: catalunya
zonas_especificas: []
estado: proxima
fecha_apertura: 2026-09-15
fecha_cierre: 2026-10-31
duracion_ventana_dias: 46
fuente_oficial: https://agaur.gencat.cat/ca/beques-i-ajuts/beca-equitat
organismo_ca: "AGAUR — Agència de Gestió d'Ajuts Universitaris"
organismo_es: AGAUR — Agencia de Gestión de Ayudas Universitarias
ultima_actualizacion: 2026-04-23
stale: false
archivada: false
draft: false
requisitos_ca:
  - Estudiar un grau o màster habilitant en universitat pública catalana
  - Renda familiar per sota del llindar 6 (≈ 60.000 €/any, depèn de la composició)
  - No tenir altra titulació universitària oficial
  - Estar matriculat/da d'un mínim de crèdits (normalment 30 ECTS)
requisitos_es:
  - Estudiar un grado o máster habilitante en universidad pública catalana
  - Renta familiar por debajo del umbral 6 (≈ 60.000 €/año, depende de composición)
  - No tener otra titulación universitaria oficial
  - Estar matriculado/a de un mínimo de créditos (normalmente 30 ECTS)
documentacion_ca:
  - DNI / NIE vigent
  - Resguard de matrícula
  - Declaració de la renda familiar
  - Llibre de família (per acreditar unitat familiar)
documentacion_es:
  - DNI / NIE vigente
  - Resguardo de matrícula
  - Declaración de la renta familiar
  - Libro de familia (para acreditar unidad familiar)
historico:
  - ano: 2025
    apertura: 2025-09-15
    cierre: 2025-10-31
    dias: 46
  - ano: 2024
    apertura: 2024-09-16
    cierre: 2024-10-31
    dias: 45
  - ano: 2023
    apertura: 2023-09-15
    cierre: 2023-10-30
    dias: 45
---

# Beca Equitat

Reducció del preu de matrícula universitària segons renda familiar.
